...
def amigo_cg_solver(args, train_loader, test_loader, val_loader):
    ...
    for epoch in range(args.epochs):
        ...
        v, outer_update, res = amigo_cg(v,parameters, hparams_yy,  hparams_xy, val_data_list, args, out_f, reg_f,epoch)
...
def amigo_cg(v,params, hparams_yy, hparams_xy, val_data_list, args, out_f, reg_f, epoch):
    '''
    ...
    v = cg_subroutine(Gy_gradient,F_y.reshape(-1),v.detach(),params,args.hessian_q)
...
def cg_subroutine(G,b,v,params,max_iter):
    '''
    Args:  
    ...
    # H_yy = construct_hessian(G, params)
    G_y_p_v = torch.matmul(G, v.detach())
    A_v = torch.autograd.grad(G_y_p_v, params, retain_graph=True)[0].reshape(-1) 
    r = A_v - b
    p = -r

    for i in range(max_iter):
        G_y_p_p = torch.matmul(G, p.detach())
