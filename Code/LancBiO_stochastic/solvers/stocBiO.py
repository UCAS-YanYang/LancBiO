...
def stocbio_solver(args, train_loader, test_loader, val_loader):
    ...
    for epoch in range(args.epochs):
        ...
        outer_update, res = stocbio(parameters, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch)
...
def stocbio(params, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch):
    '''
    ...
    v_temp = v_0
    # As stated in stocBiO, it samples a new batch for each Q-iteration
    ...
    for s in range(args.hessian_q):
        ...
        Gy_gradient = gradient_gy(args, Qsamples_labels[s], params, Qsamples_images[s], hparams_yy, output, reg_f) 
        ...
        z_list.append(v_0)            
    v_Q = args.eta*(torch.sum(torch.stack(z_list), dim=0))
