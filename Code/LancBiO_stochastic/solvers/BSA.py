import torch
from utils.dataset import nositify,build_val_data_hia
...
def bsa_solver(args, train_loader, test_loader, val_loader):
    '''
    BSA:
    ...
    for epoch in range(args.epochs):
        ...
        val_data_list, Qsamples_list, ind_yy, ind_xy = build_val_data_hia(args, val_index, images_val_list, labels_val_list, images_list, labels_list, device)
        outer_update = bsa(parameters, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch)
...
def bsa(params, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch):
    '''
    ...
    v = torch.unsqueeze(torch.reshape(Fy_gradient, [-1]), 1).detach()
    

    Qsamples_images, Qsamples_labels = Qsamples_list
    for s in range(len(Qsamples_images)):
        ...
        Gy_gradient = params.reshape(-1) - args.eta*Gy_gradient 
        G_y_p_v = torch.matmul(Gy_gradient, v)
        v = torch.autograd.grad(G_y_p_v, params, retain_graph=True)[0].reshape(-1)
        v = v.detach()
    v_Q = args.hessian_q*args.eta*v
    ...
    Gyx_gradient = torch.autograd.grad(torch.matmul(Gy_gradient, v_Q), hparams_xy)[0]
