...
def amigo_cg_solver(args, train_loader, test_loader, val_loader):
    '''
    AmIGO:  
    ...
    for epoch in range(args.epochs):
        ...
        v, outer_update,res = amigo_cg(v,parameters, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch)
...
def amigo_cg(v,params, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch):
    '''
    ...
    F_y = (torch.reshape(Fy_gradient, [-1])).detach()
    ...
    Gy_gradient = gradient_gy(args, Qsamples_labels[0], params, Qsamples_images[0], hparams_yy, output, reg_f).reshape([-1]) 
    G_y_p_v = torch.matmul(Gy_gradient, v)
    A_v = torch.autograd.grad(G_y_p_v, params, retain_graph=True)[0].reshape(-1) 
    r = A_v - F_y
    p = (-r)
    
    # As stated in AmIGO, it samples a new batch for each Q-iteration
    for s in range(1,args.hessian_q):
        ...
        G_y_p_p = torch.matmul(Gy_gradient, p.detach())
        A_p = torch.autograd.grad(G_y_p_p, params, retain_graph=True)[0].reshape(-1)   
        alpha = torch.dot(r,r)/torch.dot(p.reshape(-1),A_p)
    ...
    return v.detach(), outer_update, res
