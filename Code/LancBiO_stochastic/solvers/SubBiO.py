import torch
from utils.dataset import nositify,build_val_data_subbio
...
def subbio_solver(args, train_loader, test_loader, val_loader):
    ...
    for epoch in range(args.epochs):
        ...
        val_data_list, Qsamples_list, ind_yy, ind_xy = build_val_data_subbio(args, val_index, images_val_list, labels_val_list, images_list, labels_list, device)
        v, outer_update, res = subbio(v,parameters, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch)
...
def subbio(v,params, lambda_x, val_data_list, Qsamples_list, ind_yy, ind_xy, args, out_f, reg_f,epoch):
    ...
    hparams_yy = lambda_x[ind_yy[0]*args.batch_size: (ind_yy[0]+1)*args.batch_size]
    output = out_f(Qsamples_images[0], params)
    Gy_gradient = gradient_gy(args, Qsamples_labels[0], params, Qsamples_images[0], hparams_yy, output, reg_f) 
    ...
    # construct subspace using Schmidt orgothor
    ...
    for s in range(1,3):
        ...
        G_y_p_v = torch.matmul(Gy_gradient, W[:,s-1])
        product_temp[:,s-1] = torch.autograd.grad(G_y_p_v, params, retain_graph=True)[0].reshape(-1)
    ...
    Gyx_gradient = torch.autograd.grad(torch.matmul(Gy_gradient, v.detach()), hparams_xy)[0]
