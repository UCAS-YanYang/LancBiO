...
def lancbio_solver(args, train_loader, test_loader, val_loader):
    ...
    for epoch in range(args.epochs):
        ...
        # at the initial several epochs, we do not adopt the "Residual" and "Restart" strategies 
        # (since we do not need very accurate v^* estimation at the begining)
        # in this way, we allow the subspace dimention to achieve 300 
        if epoch < 250:
            args.dim_max = 300
            # update v, update x
            ...
            outer_update, W, F_y, q_last, lanc_beta, tridia_M, v, res = lancbio_tridiag(W, parameters, hparams_yy,  hparams_xy, val_data_list, args, out_f, reg_f,q_last,lanc_beta,tridia_M,epoch)
            weight = lambda_x[ind_xy*args.batch_size: (ind_xy+1)*args.batch_size]


        # after several epochs, we adopt the "Residual" and "Restart" strategies 
        # (since we need more accurate v^* estimation) 
        # in this way, we set subspace dimention to 10, as stated in the paper
        else:
            args.dim_max = 10
            # update v, update x
            val_index = torch.randperm(args.validation_size//args.batch_size)
            ...
            outer_update, W, F_y, q_last, lanc_beta, tridia_M, v, cor_v, Av, Av_cur, res = res_lancbio_tridiag(W, parameters, hparams_yy,  hparams_xy, val_data_list, args, out_f, reg_f,q_last,lanc_beta, tridia_M, Av, Av_cur, v, cor_v, epoch)
            weight = lambda_x[ind_xy*args.batch_size: (ind_xy+1)*args.batch_size]
...
def lancbio_tridiag(W,params, hparams_yy, hparams_xy, val_data_list, args, out_f, reg_f,q_last,lanc_beta,tridia_M,epoch):
    '''
...
def res_lancbio_tridiag(W,params, hparams_yy, hparams_xy, val_data_list, args, out_f, reg_f,q_last,lanc_beta,tridia_M,Av,Av_cur,v,cor_v,epoch):
    '''
    ...
    # restart
    if epoch==250 or (epoch%args.m==0):
        v = v + cor_v
        Gyv = torch.matmul(Gy_gradient, v)
        Av = torch.autograd.grad(Gyv, params, retain_graph=True)[0].reshape(-1,1)
        b_Av = F_y.reshape([-1,1])-Av
        W = b_Av/torch.norm(b_Av,2)
        q_last = 0
        lanc_beta = 0
        tridia_M = torch.empty(0, 0)
    ...
    q_next = q_next/lanc_beta
    q_last = q
    W = torch.cat((W, q_next), dim=1).detach()
    ...
    WTb = W[:,:-1].T@b
    sub_hess_p_Fy = torch.linalg.solve(tridia_M,WTb)
    cor_v = (W[:,:-1].reshape(7850,-1)@sub_hess_p_Fy).detach()

    v_correction = (v + cor_v).detach()

    if (epoch % args.test_fre == 0):
        G_y_p_v = torch.matmul(Gy_gradient, v_correction)
        A_v = torch.autograd.grad(G_y_p_v, params, retain_graph=True)[0].reshape(-1,1)
        res = torch.norm(A_v-F_y.reshape([-1,1]),2)
        # wandb.log({"global_step":epoch+1,"res": res})
        # print("res:",res)
    ...
    Gy_gradient = gradient_gy(args, labels_list[2], params, data_list[2], hparams_xy, output, reg_f)
    Gy_gradient = torch.reshape(Gy_gradient, [-1])
    Gyx_gradient = torch.autograd.grad(torch.matmul(Gy_gradient, v_correction), hparams_xy)[0]
    outer_update = -Gyx_gradient 
    
    return outer_update, W, F_y, q_last, lanc_beta, tridia_M, v.detach(), cor_v, Av, Av_cur, res
